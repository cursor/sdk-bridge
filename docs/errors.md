...
Older bridges mask any unexpected internal failure as a bare
`{"code":"internal","message":"internal error"}` — no `SdkErrorDetails`, no
stderr output. Nothing an adapter does can recover the underlying message
from the outside.

Current bridges attach the real failure message plus an `SdkErrorDetails`
